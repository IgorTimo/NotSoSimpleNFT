...
const { HARDHAT_PROVIDER_URL, YOUR_COLLECTIBLE } = require("../constants");
const {
  abi,
  address,
} = require("../deployments/localhost/YourCollectible.json");
...
async function main() {
  ...
  // const YC = await ethers.getContractFactory(YOUR_COLLECTIBLE, deployerSigner);
  // const yc = await YC.deploy();
  // await yc.deployed();
  // console.log('YourCollectible deployed to:', yc.address);

  // const yc = await ethers.getContract(YOUR_COLLECTIBLE, deployerSigner);
  // console.log('YourCollectible deployed to:', yc.address);
